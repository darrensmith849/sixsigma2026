  ...
  return (
    <div className="bg-white rounded-lg border border-border-grey overflow-hidden hover:shadow-md transition-shadow duration-300">
      ...
      <div className="relative aspect-[16/9] bg-light-grey">
        {imageSrc ? (
          ...
          <div className="absolute inset-0 flex items-center justify-center bg-green/10">
            <span className="text-[18px] font-semibold text-green">{title}</span>
          </div>
        )}
      </div>
      ...
      <div className="p-6">
        ...
        <p className="text-body text-[17px] leading-relaxed mb-4">
          {description}
        </p>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {modes.map((mode) => (
            <span
              key={mode}
              className="inline-block px-3 py-1 text-[14px] font-medium rounded-full bg-green/10 text-green"
            >
              {mode}
            </span>
          ))}
          ...
          <div className="mb-5">
            ...
            <ul className="space-y-1">
              {includes.map((item) => (
                <li
                  key={item}
                  className="text-[15px] text-body flex items-start gap-2"
                >
