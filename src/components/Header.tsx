    ...
    >
      <div className="container-wide">
        <div className="flex items-center justify-between h-[80px]">
          ...
          <Link href="/" className="flex items-center gap-2 shrink-0">
            <div className="flex items-center gap-2">
              <svg width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                <rect width="40" height="40" rx="8" fill="#14B24A" />
              ...
              <div className="leading-tight">
                <span className="text-[20px] font-bold tracking-tight text-sssa-black block">
                  SIX SIGMA
                </span>
                <span className="text-[11px] font-medium tracking-[0.15em] text-body-text block">
                  SOUTH AFRICA
                </span>
              </div>
            </div>
          ...
          >
            <span
              className={`block w-6 h-[2px] bg-sssa-black transition-transform duration-300 ${
                mobileOpen ? "rotate-45 translate-y-[7px]" : ""
              }`}
            />
            <span
              className={`block w-6 h-[2px] bg-sssa-black transition-opacity duration-300 ${
...
function NavItemDesktop({ item }: { item: NavItem }) {
  ...
  if (!item.children) {
    return (
      <Link
        href={item.href}
        className="px-4 py-2 text-[15px] font-medium text-sssa-black hover:text-green transition-colors"
      >
        {item.label}
      </Link>
    );
  ...
  return (
    ...
    >
      <button
        className="flex items-center gap-1 px-4 py-2 text-[15px] font-medium text-sssa-black hover:text-green transition-colors"
...
function DropdownItem({ item }: { item: SubMenuItem }) {
  ...
  if (!item.children) {
    return (
      <Link
        href={item.href}
        className="block px-4 py-2 text-[14px] text-body-text hover:text-green hover:bg-light-grey transition-colors"
      >
        {item.label}
      </Link>
    );
  ...
  return (
    ...
    >
      <button
        className="flex items-center justify-between w-full px-4 py-2 text-[14px] text-body-text hover:text-green hover:bg-light-grey transition-colors"
        onClick={() => setOpen(!open)}
      >
        {item.label}
...
function MobileNavItem({
  ...
  if (!item.children) {
    return (
      <Link
        href={item.href}
        className="block py-3 text-[16px] font-medium text-sssa-black border-b border-border-grey"
        onClick={onClose}
      >
        {item.label}
      </Link>
  ...
  return (
    <div className="border-b border-border-grey">
      <button
        className="flex items-center justify-between w-full py-3 text-[16px] font-medium text-sssa-black"
        onClick={() => setOpen(!open)}
      >
        {item.label}
        <svg
...
function MobileSubItem({
  ...
  if (!item.children) {
    return (
      <Link
        href={item.href}
        className="block py-2 text-[15px] text-body-text"
        onClick={onClose}
      >
        {item.label}
      </Link>
  ...
  return (
    <div>
      <button
        className="flex items-center justify-between w-full py-2 text-[15px] text-body-text"
        onClick={() => setOpen(!open)}
      >
        {item.label}
        <svg
